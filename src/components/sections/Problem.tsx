...
const ProblemItem = ({ icon, text, delay }: { icon: React.ReactNode, text: string, delay: number }) => {
  ...
  return (
    <div className={cn(
      "flex items-start gap-4 mb-6 transform transition-all duration-500",
...
const Problem = () => {
  ...
  return (
    <section id="problem" className="py-16" ref={sectionRef}>
      <div className="section-container">
        <h2 className={cn(
          "section-title text-center mb-12 transition-all duration-500",
          "opacity-0 translate-y-4",
        ...
        <div className="max-w-2xl mx-auto">
          <p className={cn(
            "text-xl text-center text-muted-foreground mb-12 transition-all duration-500",
            "opacity-0 translate-y-4",
            isIntersecting && "opacity-100 translate-y-0"
          )}>
          ...
          <div className="space-y-6">
            ...
            <ProblemItem 
              icon={<Palette className="w-6 h-6 text-mogency-neon-pink" />} 
              text="You're not a designer, copywriter, or marketer."
              delay={400}
            />
            
            <ProblemItem 
              icon={<Ban className="w-6 h-6 text-mogency-neon-purple" />} 
              text="You don't want to spend months building some product that might not even sell."
              delay={600}
            />
            
            <ProblemItem 
              icon={<DollarSign className="w-6 h-6 text-mogency-neon-green" />} 
